...
export function WinnerPortrait({
  ...
  if (photoUrl) {
    return (
      <div className={cn("relative overflow-hidden border-2 border-admin-line-strong", className)}>
  ...
  return (
    <div
      className={cn(
        "relative isolate overflow-hidden border-2 border-admin-line-strong",
        className,
      )}
    >
      {fallbackImage && (
      ...
      <div className="flex h-full flex-col items-center justify-center gap-5 p-10 text-center">
        <span
          aria-hidden
          className="grid h-28 w-28 place-items-center rounded-full border border-brand-onDark/60"
        >
          <span className="font-display text-6xl leading-none text-brand-onDark">
            {name.trim().charAt(0).toUpperCase()}
          </span>
        </span>
        <p className="text-eyebrow uppercase text-admin-muted">Portrait to follow</p>
      </div>
